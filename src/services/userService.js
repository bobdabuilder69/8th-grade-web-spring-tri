...
export async function getMyRequests(auth_user_id) {
    try {
        const { data, error } = await supabase
        ...
        console.error("[getMyRequests] Exception:", error);
        return [];
    }
}

// Get requests from others
export async function getNearbyRequests(auth_user_id) {
    try {
        const { data, error } = await supabase
            .from("helpNeededLocations")
            .select("*")
        ...
        if (error) throw error;
        return data.sort((a, b) => new Date(a.requestedTime) - new Date(b.requestedTime)) || [];
    } catch (error) {
        console.error("[getNearbyRequests] Exception:", error);
        return [];
    }
}
...
export async function updateRequest(id, updates) {
    try {
        ...
        if (error) {
            console.error("[updateRequest] Supabase update error:", error);
            throw error;
        }

        if (!data || data.length === 0) {
            throw new Error("Update matched 0 rows. Likely blocked by RLS");
        }
...
export async function deleteRequest(id) {
    try {
        ...
        console.error("[deleteRequest] Exception:", error);
        throw error;
    }
}

//Get accepted requests made by other users
export async function getAcceptedRequests(auth_user_id) {
    try {
        const { data, error } = await supabase
            .from("helpNeededLocations")
            .select("*")
            .contains("claimedBy", [auth_user_id])
            .neq("auth_user_id", auth_user_id);

        if (error) throw error;
        return data || [];
    } catch (error) {
        console.error("[getAcceptedRequests] Exception:", error);
        return [];
    }
}