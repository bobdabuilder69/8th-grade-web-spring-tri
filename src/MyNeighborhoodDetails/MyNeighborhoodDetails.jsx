...
//I found this function online because i didnt know how to do meter to lat/lng conversion but it works so...
function distanceMeters(a, b) {
    if (!a || !b) return Infinity;
    const toRad = (v) => (v * Math.PI) / 180;
    const [lat1, lon1] = a;
    const [lat2, lon2] = b;
    const R = 6371000; //Earth radius i think
    ...
    const aa = sinDLat * sinDLat + Math.cos(rLat1) * Math.cos(rLat2) * sinDLon * sinDLon;
...
function extractLatLng(geo) {
    ...
    if (geo.lat != null && geo.lng != null) return [Number(geo.lat), Number(geo.lng)];
    if (geo.latitude != null && geo.longitude != null) return [Number(geo.latitude), Number(geo.longitude)];
    return null;
}
...
export default function MyNeighborhoodDetails() {
    ...
    useEffect(() => {
        async function loadUser() {
            ...
            if (user?.auth_user_id) {
                ...
                console.log("[MyNeighborhoodDetails] myRequests count:", (mine || []).length, "nearbyRequests count:", (nearby || []).length);
            }
    ...
    async function updateRadiusDb(newRadius) {
        if (matchedUser?.auth_user_id) {
            try {
                ...
                console.error("[MyNeighborhoodDetails] Error saving radius:", err);
            }
        }
    }
    ...
    async function saveEdit() {
        try {
            ...
            setMyRequests((prev) =>
                prev.map((r) => (r.requestId === editingId ? { ...r, ...updated } : r)),
            );
            cancelEdit();
        } catch (err) {
    ...
    async function handleDelete() {
        ...
        try {
            ...
            setMyRequests((prev) => prev.filter((r) => r.requestId !== editingId));
            cancelEdit();
        } catch (err) {
        ...
        <div className="container">
            ...
            {matchedUser ? (
                <div className="listItem">
                    <h2>{capitalizeFirstLetter(matchedUser.name.replace(/\b\w/g, char => char.toUpperCase()))}</h2>
                    <div id="columnMaker">
                        <div id="mapContainer">
                            ...
                            <input
                                ...
                                onMouseUp={(e) => updateRadiusDb(Number(e.target.value))}
                            />
                        </div>
                        ...
                        <div className="columnContainer">
                            <div className="columnRowContainer">
                                ...
                                {(() => {
                                            ...
                                            console.log("[MyNeighborhoodDetails] center:", center, "radius:", radius);
                                            console.log("[MyNeighborhoodDetails] nearbyRequests count:", (nearbyRequests || []).length);
                                            var visibleNearby = (nearbyRequests || []).filter((req) => {
                                                const rloc = extractLatLng(req.geolocation);
                                                if (!center || !rloc) return false;
                                                const d = distanceMeters(center, rloc);
                                                console.log("[MyNeighborhoodDetails] req", req.requestId, "loc:", rloc, "dist_m:", Math.round(d));
                                                return d <= Number(radius || 0);
                                            });

                                            if (visibleNearby.length === 0) {
                                                return <p>No nearby requests yet.</p>;
                                            }
                                                ...
                                                <ul className="nearbyRequestContainer">
                                                    {visibleNearby.map((req) => (
                                                        <li className="nearbyRequestItem" key={req.requestId}>
                                                            ...
                                                            <small>
                                                                {req.timeEstimate} min &middot; {req.address}{" "}
